...
public static class SettingFileProcessing
{
    ...
    public static bool ReadSettings()
    {
            ...
            && path.Contains(SettingsValue.OldSettingFileExtension))
        {
            ...
            if (directoryName != null && fileNameWithoutExtension != null)
            {
                ...
                {
                    // 旧設定ファイルの削除確認をして削除
                    if (MessageBox.Show(ApplicationData.Strings.NewSettingFile, ApplicationData.Strings.Check + WindowControlValue.CopySeparateString + ApplicationValue.ApplicationName, MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                    {
                        try
                        {
                            File.Delete(path);
                            MessageBox.Show(ApplicationData.Strings.Deleted, ApplicationData.Strings.Check + WindowControlValue.CopySeparateString + ApplicationValue.ApplicationName, MessageBoxButton.OK);
                        }
                        catch
                        {
                            MessageBox.Show(ApplicationData.Strings.ErrorOccurred, ApplicationData.Strings.Check + WindowControlValue.CopySeparateString + ApplicationValue.ApplicationName, MessageBoxButton.OK);
                        }
                    }
                }
            }
    ...
    private static bool CreateSettingFile()
    {
        if (string.IsNullOrEmpty(ApplicationData.SpecifySettingsFilePath))
        {
            ...
            if (ApplicationPath.CheckInstalled())
            {
                ...
                {
                    switch (FEMessageBox.Show(ApplicationData.Strings.ShareSettings, ApplicationData.Strings.Check + WindowControlValue.CopySeparateString + ApplicationValue.ApplicationName, MessageBoxButton.YesNo))
                    {
        ...
        )
    {
        ...
        try
        {
            string writeString = JsonSerializer.Serialize(ApplicationData.Settings, FileProcessing.JsonSerializerOptions);
        ...
        )
    {
        ...
        try
        {
            if (File.Exists(path))
            {
                ...
                Settings? settings = JsonSerializer.Deserialize<Settings>(readString, FileProcessing.JsonSerializerOptions);
        ...
        )
    {
        ...
        try
        {
            ...
            if (element != null)
            {
                ...
                elementNode1 = element.Element("SpecifyWindowInformation") ?? element.Element("EventInformation");
                if (elementNode1 != null)
                {
