...
namespace Swindom.Source
{
    ...
    /// ホットキー処理
    /// </summary>
    public class HotkeyProcessing : System.IDisposable
    {
        /// <summary>
        /// Disposeが呼ばれたかの値
        /// </summary>
        ...
        {
            return (Common.ApplicationData.Settings.HotkeyInformation.Enabled
            ...
            )
        {
            try
            {
                ...
                foreach (HotkeyItemInformation nowHII in Common.ApplicationData.Settings.HotkeyInformation.Items)
                {
                    ...
                    {
                        PerformHotkeyProcessing(hwnd, nowHII, monitorInformation);
                        break;
                    }
                }
            }
                ...
                default:
                    ...
                    switch (hotkeyItemInformation.ProcessingType)
                    {
                        case HotkeyProcessingType.PositionSize:
                            {
                                RectangleDecimal windowRectangle = WindowProcessing.CalculatePositionAndSize(hwnd, windowPlacement, hotkeyItemInformation.PositionSize, hotkeyItemInformation.StandardDisplay, hotkeyItemInformation.PositionSize.ClientArea, null);
                                if (monitorInformation == null)
                                {
                                    monitorInformation = MonitorInformation.GetMonitorInformation();
                                }
                                if (Common.ApplicationData.Settings.HotkeyInformation.DoNotChangeOutOfScreen == false || WindowProcessing.CheckWindowIsInTheScreen(windowRectangle, monitorInformation))
                                {
                        ...
                        case HotkeyProcessingType.MoveXY:
                            NativeMethods.SetWindowPos(hwnd, 0, windowPlacement.rcNormalPosition.Left + hotkeyItemInformation.ProcessingValue, windowPlacement.rcNormalPosition.Top + hotkeyItemInformation.ProcessingValue, 0, 0, (int)SWP.SWP_NOSIZE | (int)SWP.SWP_NOZORDER);
                            break;
