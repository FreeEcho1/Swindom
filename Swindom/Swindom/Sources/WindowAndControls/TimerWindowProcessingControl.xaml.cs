...
/// </summary>
public partial class TimerWindowProcessingControl : UserControl, IDisposable
{
    /// <summary>
    /// Disposeが呼ばれたかの値
    /// </summary>
        ...
        )
    {
        try
        {
            ...
            {
                AddModifyWindowSpecifiedWindow = new(Window.GetWindow(this), Common.ApplicationData.Settings.TimerInformation);
        ...
        )
    {
        try
        {
            ...
            else if (TimerListBox.SelectedItems.Count == 1)
            {
                AddModifyWindowSpecifiedWindow = new(Window.GetWindow(this), Common.ApplicationData.Settings.TimerInformation, TimerListBox.SelectedIndex);
                AddModifyWindowSpecifiedWindow.Closed += AddModifyWindowSpecifiedWindow_Closed;
                AddModifyWindowSpecifiedWindow.Show();
            }
        }
        ...
        )
    {
        try
        {
            if (TimerListBox.SelectedItems.Count == 1
                && FreeEcho.FEControls.MessageBox.Show(Common.ApplicationData.Languages.Check, Common.ApplicationData.Settings.TimerInformation.Items[TimerListBox.SelectedIndex].RegisteredName + Environment.NewLine + Common.ApplicationData.Languages.LanguagesWindow?.AllowDelete, FreeEcho.FEControls.MessageBoxButton.YesNo, Window.GetWindow(this)) == FreeEcho.FEControls.MessageBoxResult.Yes)
            {
        ...
        )
    {
        try
        {
            if (TimerListBox.SelectedItems.Count == 1)
            {
                ...
                {
                    if (Common.ApplicationData.Settings.TimerInformation.Items[count].RegisteredName == (newTII.RegisteredName + Common.CopySeparateString + Common.ApplicationData.Languages.LanguagesWindow?.Copy + Common.SpaceSeparateString + number))
                    {
                        count = 0;
                        number++;
                    }
                }
                newTII.RegisteredName += Common.CopySeparateString + Common.ApplicationData.Languages.LanguagesWindow?.Copy + Common.SpaceSeparateString + number;
                int selectedIndex = TimerListBox.SelectedIndex;      // 選択している項目のインデックス
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void TimerListBox_Item_CheckStateChanged(
        object sender,
    ...
    private void SetTextOnControls()
    {
        try
        {
            ...
            {
                throw new Exception("LanguagesWindow value is null. - TimerWindowProcessingControl.SetTextOnControls()");
            }
            ...
            ((ComboBoxItem)ProcessingWindowRangeComboBox.Items[0]).Content = Common.ApplicationData.Languages.LanguagesWindow.ActiveWindowOnly;
            ((ComboBoxItem)ProcessingWindowRangeComboBox.Items[1]).Content = Common.ApplicationData.Languages.LanguagesWindow.AllWindow;
    ...
    private void UpdateTimerListBoxItems()
    {
        ...
        if (Common.ApplicationData.Settings.TimerInformation.Items.Count != 0)
        {
            foreach (TimerItemInformation nowTII in Common.ApplicationData.Settings.TimerInformation.Items)
            {
                ...
                newItem.CheckStateChanged += TimerListBox_Item_CheckStateChanged;
