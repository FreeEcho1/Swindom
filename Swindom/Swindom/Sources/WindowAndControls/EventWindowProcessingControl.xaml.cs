...
/// </summary>
public partial class EventWindowProcessingControl : UserControl, IDisposable
{
    /// <summary>
    /// Disposeが呼ばれたかの値
    /// </summary>
        ...
        )
    {
        try
        {
            ...
            {
                AddModifyWindowSpecifiedWindow = new(Window.GetWindow(this), Common.ApplicationData.Settings.EventInformation);
        ...
        )
    {
        try
        {
            if (AddModifyWindowSpecifiedWindow == null)
            {
                ...
                {
                    AddModifyWindowSpecifiedWindow = new(Window.GetWindow(this), Common.ApplicationData.Settings.EventInformation, EventListBox.SelectedIndex);
                    AddModifyWindowSpecifiedWindow.Closed += AddModifyWindowSpecifiedWindow_Closed;
                    AddModifyWindowSpecifiedWindow.Show();
                }
            }
        ...
        )
    {
        try
        {
            if (EventListBox.SelectedItems.Count == 1)
            {
                if (FreeEcho.FEControls.MessageBox.Show(Common.ApplicationData.Languages.Check, Common.ApplicationData.Settings.EventInformation.Items[EventListBox.SelectedIndex].RegisteredName + Environment.NewLine + Common.ApplicationData.Languages.LanguagesWindow?.AllowDelete, FreeEcho.FEControls.MessageBoxButton.YesNo, Window.GetWindow(this)) == FreeEcho.FEControls.MessageBoxResult.Yes)
                {
        ...
        )
    {
        try
        {
            if (EventListBox.SelectedItems.Count == 1)
            {
                ...
                {
                    if (Common.ApplicationData.Settings.EventInformation.Items[count].RegisteredName == (newItem.RegisteredName + Common.CopySeparateString + Common.ApplicationData.Languages.LanguagesWindow?.Copy + Common.SpaceSeparateString + number))
                    {
                        // 番号を変えて最初から確認
                        count = 0;
                        number++;
                ...
                newItem.RegisteredName += Common.CopySeparateString + Common.ApplicationData.Languages.LanguagesWindow?.Copy + Common.SpaceSeparateString + number;
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void EventListBox_Item_CheckStateChanged(
        object sender,
    ...
    private void SetTextOnControls()
    {
        try
        {
            ...
            {
                throw new Exception("LanguagesWindow value is null. - EventWindowProcessingControl.SetTextOnControls()");
            }
    ...
    private void UpdateEventListBoxItems()
    {
        ...
        if (Common.ApplicationData.Settings.EventInformation.Items.Count != 0)
        {
            foreach (EventItemInformation nowEII in Common.ApplicationData.Settings.EventInformation.Items)
            {
                ...
                newItem.CheckStateChanged += EventListBox_Item_CheckStateChanged;
