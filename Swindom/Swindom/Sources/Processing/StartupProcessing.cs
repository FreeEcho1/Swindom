...
public static class StartupProcessing
{
    ...
    /// タスクを作成
    /// </summary>
    public const string CreateTaskString = "CreateTask";
    /// <summary>
    /// タスクを削除
    /// </summary>
    public const string DeleteTaskString = "DeleteTask";

    /// <summary>
    /// スタートアップのショートカットファイルのパスを取得
    /// </summary>
    /// <returns>スタートアップのショートカットファイルのパス</returns>
    private static string StartupFilePath() => Environment.GetFolderPath(Environment.SpecialFolder.Startup) + Path.DirectorySeparatorChar + Common.ApplicationName + ".lnk";
    ...
    public static void RegisterStartup()
    {
        if (CheckStartup() == false)
        {
            ...
            shortcut.TargetPath = VariousProcessing.OwnApplicationPath();
            shortcut.Save();
        }
    }
    ...
    public static void RegisterTask()
    {
        ...
        try
        {
            ...
            execAction.Path = VariousProcessing.OwnApplicationPath();
            ...
            try
            {
                rootFolder.RegisterTaskDefinition(Common.ApplicationName, taskDefinition, (int)TaskScheduler._TASK_CREATION.TASK_CREATE_OR_UPDATE, null, null, TaskScheduler._TASK_LOGON_TYPE.TASK_LOGON_NONE, null);
            }
            finally
            {
    ...
    public static void DeleteTask()
    {
        ...
        try
        {
            ...
            try
            {
                rootFolder.DeleteTask(Common.ApplicationName, 0);
            }
            finally
            {
    ...
    public static bool CheckTaskRegistered()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    if (nowTaskFolder.Name == Common.ApplicationName)
                    {
                        result = true;
                        break;
                    }
